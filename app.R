...
##Once all transition-related inputs are provided, we confirm and proceed
				...
				fluidRow(
					...
					column(
						...
						selectInput(
							...
							choices = c(setNames(seq_along(strata_labels), strata_labels),
													pooled = "pooled")
						)
					)
				),
	...
	filtered_data <- reactive({
		...
		if (identical(input$strata_plot, "pooled")) {
			# Best: pooled is a named element
			if (!is.null(names(res_scn)) && "pooled" %in% names(res_scn)) {
				strata_obj <- res_scn[["pooled"]]
			} else {
				# Fallback: pooled is last element by convention
				strata_obj <- res_scn[[length(res_scn)]]
			}
		} else {
			i_strata <- as.integer(input$strata_plot)
			strata_obj <- res_scn[[i_strata]]
		}
		
		cat("len(strata_labels) =", length(SI_rvs$strata_labels),
				" len(res_scn) =", length(res_scn),
				" strata_plot =", input$strata_plot, "\n")
		
		req(strata_obj$state_probabilities)
		strata_obj$state_probabilities
	})
